...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a binary multi-view gated-image dataset from view-organized true-3D "
            "and flat-false render roots. View labels are included in output sample IDs "
            "to prevent different views of the same model from overwriting each other."
        )
    )
    ...
    parser.add_argument("--expected-views", type=int, default=0)
...
def has_gate_pngs(path: Path) -> bool:
    return any(GATE_PATTERN.match(child.name) for child in path.rglob("*.png"))


def discover_view_dirs(root: Path) -> list[tuple[str, Path]]:
    direct_view_dirs = [
        path
        for path in sorted(root.iterdir())
        if path.is_dir()
        and not path.name.startswith("_")
        and path.name.lower().startswith("view")
        and has_gate_pngs(path)
    ]
    if direct_view_dirs:
        return [(path.name, path) for path in direct_view_dirs]
    if has_gate_pngs(root):
        return [("view0", root)]
    return []


def grouped_samples(view_dir: Path, view_id: str, source_kind: str, expected_num_slices: int) -> list[GateSample]:
    samples: list[GateSample] = []
    for class_dir in sorted(path for path in view_dir.iterdir() if path.is_dir() and not path.name.startswith("_")):
        ...
        for sample_id, paths in sorted(grouped.items()):
            if len(paths) != expected_num_slices:
                continue
            samples.append(GateSample(source_kind, view_id, class_dir.name, sample_id, paths))
    return samples
...
def copy_samples(samples: list[GateSample], class_name: str, output_root: Path) -> list[dict[str, object]]:
    ...
    for sample in samples:
        output_base = f"{slug(sample.view_id)}__{slug(sample.source_class)}__{slug(sample.sample_id)}"
...
def collect_samples(root: Path, source_kind: str, expected_num_slices: int, expected_views: int) -> list[GateSample]:
    view_dirs = discover_view_dirs(root)
    if not view_dirs:
        raise RuntimeError(f"No view directories with gated PNGs found under {root}")
    if expected_views > 0 and len(view_dirs) != expected_views:
        raise RuntimeError(f"Expected {expected_views} views under {root}, found {len(view_dirs)}")
    samples: list[GateSample] = []
    for view_id, view_dir in view_dirs:
        samples.extend(grouped_samples(view_dir, view_id, source_kind, expected_num_slices))
    return samples
...
def main() -> int:
    ...
    true_samples = collect_samples(args.true_root, "true3d", args.expected_num_slices, args.expected_views)
    false_samples = collect_samples(args.false_root, "flat_false", args.expected_num_slices, args.expected_views)
    if not true_samples:
    ...
    print(f"views_true={len({sample.view_id for sample in true_samples})}")
    print(f"views_false={len({sample.view_id for sample in false_samples})}")
